...
# --- Configuration ---
# !!!!! IMPORTANT !!!!!
# SET THIS PATH to the directory containing your class subfolders (cardboard, glass, etc.)
dataset_path = '/home/ubuntu/Jupiter_dl/Final_project/dataset-resized' # <--- CHANGE THIS LINE

# --- Sanity Check ---
if not os.path.isdir(dataset_path):
    print(f"ERROR: The specified dataset path does not exist or is not a directory:")
    print(f"  '{dataset_path}'")
    print("Please ensure the path is correct and points to the folder containing the class subdirectories (e.g., 'cardboard', 'glass', etc.).")
    sys.exit(1) # Stop the script if the path is invalid

# Check if it looks like a dataset directory (contains subdirs)
try:
    if not any(os.path.isdir(os.path.join(dataset_path, item)) for item in os.listdir(dataset_path)):
        print(f"WARNING: The directory '{dataset_path}' exists, but doesn't seem to contain subdirectories.")
        print("ImageDataGenerator expects subdirectories named after your classes ('cardboard', 'glass', etc.) inside this path.")
        # Decide if you want to exit or proceed with caution
        # sys.exit(1)
except Exception as e:
     print(f"ERROR: Could not access contents of the directory '{dataset_path}'. Check permissions.")
     print(f"Details: {e}")
     sys.exit(1)
